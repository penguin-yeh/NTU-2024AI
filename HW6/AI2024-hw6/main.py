...
def parse_args():
    ...
    parser.add_argument("--num_epochs", type=int, default=1, choices=[1, 3, 5])
...
if __name__ == "__main__":
    ...
    if args.inference_base_model:
        if args.model_name == "unsloth/llama-3-8b-bnb-4bit":
            ...
            inference.LLM_inference(args)
        elif args.model_name == "unsloth/llama-3-8b-Instruct-bnb-4bit":
            print("Inference with base model: unsloth/llama-3-8b-Instruct-bnb-4bit")
            inference.LLM_inference(args)
        else:
