...
def depthFirstSearch(problem: SearchProblem):
    ...
    while not frontier.isEmpty():  
        ...
        if len(cur_node) == 1:
            ...
            # print(cur_node)
            # print(type(cur_node[0]) == int)
            if type(cur_node[0]) == int: # just for starting point
                cur_coor = (cur_node[0], cur_node[1])
                cur_action = 0
        ...
        if problem.isGoalState(cur_coor):
            print("finished!!")
            break # finish, find goal_node(which is cur_node)
        explored_set.append(cur_coor)
        # print(cur_coor)
        # print(cur_action)
        successor = problem.getSuccessors(cur_coor)
        ...
        for i in range(successor_len):
            ...
            # print(next)
            # print(next)
            # if cur_node == problem.getStartState(): # for starting node
            #     next_tmp = ()
            #     next_tmp += (next[0], 'starting', )
            # else:
            next_tmp = ()
            next_tmp += (next[0], next[1],)
            
            if parents.get(next_tmp) is None:
                if len(cur_node) == 1: # for starting node
                    cur_pairs = ()
                    cur_pairs += (cur_coor, 'starting')
                    parents[next_tmp] = cur_pairs 
                elif len(cur_node) == 2:
                    cur_pairs = ()
                    cur_pairs += (cur_coor, cur_action)
                    parents[next_tmp] = cur_pairs 
            ...
            # if problem.isGoalState(next_tmp):
    ...
    while cur_node[0] != problem.getStartState():
        ...
        if explored_flag is True:
            ...
            print(parents[cur_node])
            goal_path.insert(0, cur_node[1])
            # next_str = cur_node + actions[cur_node]
            # print(next_str)
